...
function Publish() {
    ...
    const handleChangeColor = (index: number) => (e: React.MouseEvent<HTMLParagraphElement>) => {
        setIsClicked(isClicked.map((clicked, i) => i === index ? !clicked : clicked));
    }    
    ...
    <div className="bg-lightColor flex justify-center items-center border-b-[1px] mt-[0.02rem] py-[1rem] px-[2rem] md:px-[5rem]">
        <form className="search flex gap-4 w-full py-2 bg-lightColor rounded-full border-[1px] border-gray-200">
            <div className="flex flex-col gap-2 items-center w-[40%] ms-4 md:px-0 md:flex-row">
                ...
                <input type="text" name="keywords" className="w-full px-2 focus:outline-none" placeholder="Titre d’emploi ou mot clé" />
            </div>
            ...
            <hr className="vertical-line w-[2px] h-12 bg-black me-7" />
                ...
                <input type="text" name="localisation" className="w-full px-2 focus:outline-none" placeholder="Préciser la localisation" />
            </div>
        ...
        <div className="h-auto bg-lightColor rounded-sm border-[1px] border-gray-200 sticky top-4 px-5">
            ...
            <ul className="grid grid-cols-[repeat(auto-fill,minmax(12rem,1fr))] gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-3">
                <li className="flex flex-col w-full">
                    <label htmlFor="society-name">Nom de la sociéte</label>
                ...
                <li className="flex flex-col w-full">
                    ...
                    <input type="text" name="logo" id="logo" className="w-full rounded-sm bg-slate-50 h-8 mt-2 mb-8 px-3 focus:outline-none" placeholder="Ecrivez..." />
                </li>
            </ul>
            <ul className="grid grid-cols-[repeat(auto-fill,minmax(12rem,1fr))] gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 mt-2">
                <li className="flex flex-col w-full">
                    ...
                    <ul>
                        <li className="grid grid-cols-2 gap-3 mt-2">
                            <p className={`flex justify-center border-[1px] rounded-full p-2 ${isClicked[0] ? 'bg-blue-600 text-white' : 'bg-white'}`} onClick={handleChangeColor(0)}>Commercial</p>
                            <p className={`flex justify-center border-[1px] rounded-full p-2 ${isClicked[1] ? 'bg-blue-600 text-white' : 'bg-white'}`} onClick={handleChangeColor(1)}>Environnement</p>
                            <p className={`flex justify-center border-[1px] rounded-full p-2 ${isClicked[2] ? 'bg-blue-600 text-white' : 'bg-white'}`} onClick={handleChangeColor(2)}>Developpement</p>
                            <p className={`flex justify-center border-[1px] rounded-full p-2 ${isClicked[3] ? 'bg-blue-600 text-white' : 'bg-white'}`} onClick={handleChangeColor(3)}>Marketing</p>
                        </li>
                ...
                <li className="col-span-2 flex lg:flex-row md:flex-col sm:flex-col">
                    <div className="flex flex-col lg:w-[50%] sm:w-[80%]">
                        <label htmlFor="society-name sm:my-2">Description du poste</label>
                        <textarea name="description" cols={80} rows={10} className="bg-slate-50 focus:outline-none p-2"></textarea>
                    </div>
